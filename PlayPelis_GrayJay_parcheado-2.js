...
var _mediaOk = {};      /* host de CDN de video -> timestamp del ultimo probe sano (ver hardenSrc) */
var MEDIA_OK_TTL_MS = 600000; /* 10 min: mientras dure, se confia en el host sin re-probarlo */
...
/* ------------------------------------------------------------------ */
/* verificacion de fuentes ya resueltas: 3 fallas tipicas de los CDN    */
/* firmados (acek-cdn, dramiyos-cdn, cloudwindow-route, etc.):          */
/*   (2) Referer/Origin que el edge no acepta -> se puede probar y      */
/*       corregir en el momento.                                       */
/*   (3) nodo (srv=) caido/saturado -> se puede probar y volver a       */
/*       resolver el mismo embed para pedir uno nuevo.                  */
/*   (1) token de un solo uso ya gastado -> NO se puede "probar" sin    */
/*       arriesgarse a ser justamente el uso que lo consume; para esta  */
/*       unica causa el parche es evitar el chequeo en links con pinta  */
/*       de firmados y en cambio asegurar 2+ fuentes independientes.    */
/* ------------------------------------------------------------------ */

/* apagado por defecto (opt-in con settings.verifySources=true): no esta
   confirmado como se comporta http.GET() con "Range" contra estos CDN en este
   entorno, asi que hasta probarlo no conviene que corra solo. */
function verifySources() { var v = _settings && _settings.verifySources; return v === true || v === "true" || v === 1 || v === "1"; }
var REFERER_TRIES_MAX = 3;
var SIGNED_URL_RE = /[?&]t=[^&]+.*[?&]e=\d+|[?&]e=\d+.*[?&]t=[^&]+/i; /* token+expiry en la query = pinta de "de un solo uso" */

function looksLikeErrorPage(body) {
    if (!body) return true;
    var head = body.substring(0, 300).toLowerCase();
    return head.indexOf("<html") >= 0 || head.indexOf("<!doctype") >= 0;
}
/* pide el propio manifest/segmento con los MISMOS headers que usaria GrayJay
   al reproducir (Referer/Origin/UA + Range corto para no bajar el archivo
   entero), para detectar ANTES de entregarle la fuente si el nodo esta caido
   (502/504/404/HTML) o si el Referer/Origin no es el que el edge espera (403).
   No reutiliza httpGet() porque ese tiene su propio "host caido" pensado para
   paginas de scraping, no para el archivo de video final. */
function probeMedia(url, ref) {
    var h = { "User-Agent": UA, "Accept": "*/*", "Range": "bytes=0-2048" };
    if (ref) { h["Referer"] = ref; var o = originOf(ref); if (o) h["Origin"] = o; }
    try {
        var r = http.GET(url, h, false), b = readBody(r), code = r ? r.code : 0;
        return { code: code, ok: (code == 200 || code == 206) && !looksLikeErrorPage(b) };
    } catch (e) { return { code: 0, ok: false }; }
}
/* prueba el Referer original, despues el origen del propio link, despues
   ninguno -algunos edges rechazan justamente un Referer "de mas"-. Si alguno
   responde bien, ese es el que hay que usar de ahi en adelante para esa
   fuente. Si el fallo es de red/servidor (5xx/404/sin respuesta) corta apenas
   lo detecta: no es un problema de Referer, es nodo caido (causa 3). */
function findWorkingReferer(url, ref) {
    var tries = [], seen = {}, i;
    function push(r) { var k = r || "(none)"; if (!seen[k]) { seen[k] = 1; tries.push(r); } }
    push(ref); push(originOf(url) + "/"); push("");
    for (i = 0; i < tries.length && i < REFERER_TRIES_MAX; i++) {
        var res = probeMedia(url, tries[i] || null);
        if (res.ok) return tries[i];
        if (res.code >= 500 || res.code === 404 || res.code === 0) return null;
    }
    return null;
}
/* endurece una fuente ya resuelta (HLSSource/VideoUrlSource). "reresolve" es
   una funcion que vuelve a pedir el MISMO embed desde cero (nueva request al
   player.php/callistanise/etc.), lo que en la practica entrega un token y/o
   nodo nuevo -sirve igual para "nodo caido" (3) y para renovar un token que ya
   se gasto (1), sin necesidad de distinguir cual de las dos fue. */
function hardenSrc(c, s, reresolve) {
    if (!s || !s.url) return s;
    if (SIGNED_URL_RE.test(s.url)) { log("    verificar: link firmado, se omite el chequeo (evitar gastarlo)"); return s; }
    var h0 = hostOf(s.url);
    if (_mediaOk[h0] && (Date.now() - _mediaOk[h0]) < MEDIA_OK_TTL_MS) { log("    verificar: host " + h0 + " confirmado sano hace poco, se omite el chequeo"); return s; }
    var ref = (s.requestModifier && s.requestModifier.headers && s.requestModifier.headers["Referer"]) || c.ref;
    var first = probeMedia(s.url, ref);
    if (first.ok) { _mediaOk[h0] = Date.now(); return s; }
    log("    verificar: " + (first.code || "sin respuesta") + " en " + prettyHost(s.url) + ", reintentando");
    if (first.code == 403) {
        var okRef = findWorkingReferer(s.url, ref);
        if (okRef !== null) { s.requestModifier = reqMod(okRef || (originOf(s.url) + "/")); _mediaOk[h0] = Date.now(); log("    verificar: referer alternativo funciono"); return s; }
    }
    markFail(h0);
    ...
    /* IMPORTANTE: nunca se devuelve null aca. Si el chequeo no logro confirmar
       ni corregir la fuente, se devuelve la original SIN TOCAR -si el chequeo
       esta equivocado (falso negativo del propio probeMedia en este entorno),
       el resultado es identico a no haber verificado nada, nunca peor. */
    log("    verificar: no se pudo confirmar, se deja la fuente original sin tocar");
    return s;
}
...
function resolveCands(cands, out, prov) {
    ...
    for (i = 0; i < list.length && n < MAX_CAND && budgetLeft(); i++) {
        ...
        if (c.srcs) {
            ...
            /* endurecer cada fuente resuelta (parches referer/nodo caido) antes
               de sumarla; si hace falta, "reresolve" vuelve a pedir el mismo
               embed desde cero para conseguir un token/nodo nuevo. */
            if (verifySources() && got.length && budgetLeft()) {
                var reresolve = (function (cand, lbl) {
                    return function () { return resolveEmbed(cand.url, lbl, cand.ref || (originOf(cand.url) + "/"), 0); };
                })(c, label);
                var hardened = [];
                for (j = 0; j < got.length; j++) {
                    var hs = hardenSrc(c, got[j], reresolve);
                    if (hs) hardened.push(hs);
                }
                got = hardened;
            }
        }
...
/* cuenta hosts de CDN distintos entre las fuentes ya conseguidas. Sirve para
   no cortar la busqueda con "suficientes servidores" cuando en realidad todo
   lo encontrado depende de un unico pool de CDN (si ese pool esta inestable,
   causa 3, todas las fuentes fallarian juntas aunque parezcan varias). */
function distinctHosts(arr) {
    var seen = {}, i, h, n = 0;
    for (i = 0; i < arr.length; i++) { h = hostOf(arr[i].url); if (h && !seen[h]) { seen[h] = 1; n++; } }
    return n;
}
function collectSources(ctx) {
    ...
    for (i = 0; i < plan.length; i++) {
        ...
        if (servers >= WANT_SERVERS && distinctHosts(out) >= 2 && !extraSites()) { log("Suficientes servidores (" + servers + ") en " + distinctHosts(out) + " hosts distintos, no se buscan mas sitios"); break; }
        log("> " + plan[i].n);
        try {
            var c = plan[i].f(ctx);
...
if (typeof source != "undefined") {
    source.enable = function (conf, settings, savedState) { _settings = settings || {}; _fail = {}; _okh = {}; _pre = {}; _mediaOk = {}; };
