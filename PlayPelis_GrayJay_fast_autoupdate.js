...
var MAX_CAND = 8;          /* embeds por proveedor; se cortan al hallar una fuente REAL */
var WANT_SERVERS = 1;      /* 1 proveedor con fuente REAL; no se corta por un embed muerto,
...
var WANT_CANDS = 1;        /* basta 1 fuente reproducible; si falla se sigue con el siguiente servidor */
...
function verifySources() { var v = _settings && _settings.verifySources; if (v === false || v === "false" || v === 0 || v === "0") return false; return true; }   /* validacion rapida activa por defecto */
function debugMode() { var v = _settings && _settings.debugMode; return v === true || v === "true" || v === 1 || v === "1"; }

/* ------------------------------------------------------------------ */
/* AUTO-DOMINIOS: autoconfiguracion interna del JS                    */
/* Solo cambia el dominio usado cuando el actual deja de funcionar.  */
/* No reescribe este archivo ni depende de Python/GitHub.             */
/* ------------------------------------------------------------------ */
var AUTO_DOMAIN_TTL = 21600000; /* 6 horas */
var _autoDomains = {};
var _autoDomainAt = {};
var AUTO_DOMAIN_ALTS = {
    poseidonhd: ["https://www.poseidonhd2.co", "https://poseidonhd.nz", "https://poseidonhd.tv"],
    cuevana3e: ["https://cuevana3e.pro", "https://cuevana3e.tv", "https://cuevana3.biz"],
    juanita: ["https://pelisjuanita.com", "https://pelisjuanita.tv"]
};
function autoDomainBase(id, fallback) {
    return _autoDomains[id] || fallback || "";
}
function autoDomainProbe(id, base) {
    var r, code, h, low;
    if (!base || !budgetLeft()) return false;
    try {
        r = http.GET(String(base).replace(/\/+$/, "") + "/", {"User-Agent": UA, "Accept": "text/html,application/xhtml+xml"}, false);
        code = r && r.code ? r.code : 0;
        h = readBody(r);
        low = String(h || "").toLowerCase();
        if (code < 200 || code >= 400 || !h) return false;
        if (/(cloudflare|access denied|forbidden|captcha required|just a moment)/i.test(low)) return false;
        if (id == "juanita" && code == 403) return false;
        if (id == "poseidonhd") return /(?:<html|<body|wordpress|wp-content|pelicula|serie|search)/i.test(h);
        if (id == "cuevana3e") return /(?:<html|<body|cuevana|pelicula|serie|search|data-server)/i.test(h);
        if (id == "juanita") return /(?:<html|<body|pelicula|serie|movie|video)/i.test(h);
    } catch (e) { log("AUTO " + id + " probe -> " + e); }
    return false;
}
function autoDomainEnsure(id, current) {
    var list = [], i, v, now = Date.now();
    if (_autoDomains[id] && _autoDomainAt[id] && now - _autoDomainAt[id] < AUTO_DOMAIN_TTL) return _autoDomains[id];
    v = _autoDomains[id] || current || "";
    if (v) list.push(v);
    for (i = 0; i < (AUTO_DOMAIN_ALTS[id] || []).length; i++) if (list.indexOf(AUTO_DOMAIN_ALTS[id][i]) < 0) list.push(AUTO_DOMAIN_ALTS[id][i]);
    /* Primero se comprueba el dominio que ya estaba usando. Si funciona, NO se cambia. */
    if (list.length && autoDomainProbe(id, list[0])) {
        _autoDomains[id] = list[0]; _autoDomainAt[id] = now;
        return list[0];
    }
    /* Solo ante fallo real se prueban las alternativas conocidas. */
    for (i = 1; i < list.length && budgetLeft(); i++) if (autoDomainProbe(id, list[i])) {
        _autoDomains[id] = list[i]; _autoDomainAt[id] = now;
        log("AUTO " + id + ": dominio cambiado a " + list[i]);
        return list[i];
    }
    /* Ninguna alternativa demostro ser funcional: no inventar ni cambiar. */
    if (v) { _autoDomains[id] = v; _autoDomainAt[id] = now; log("AUTO " + id + ": sin alternativa funcional; se conserva " + v); }
    return v;
}
function autoDomainsInit() {
    var v = _settings && _settings.autoDomains;
    if (v === false || v === "false" || v === 0 || v === "0") return;
    /* La validacion es lazy: el dominio se comprueba cuando el proveedor se usa. */
    log("AUTO-DOMINIOS interno activo");
}
...
function discoverLinks(html, pageUrl) {
    ...
    /* Cuevana3e/tungtungsahur: la URL real puede estar dentro de atob("base64...").
       Hay que decodificarla ANTES de buscar enlaces; de lo contrario el scraper ve solo
       https://tungtungsahur.../?token= y nunca llega a vsembed/vidlink/vidapi. */
    re = /atob\(\s*["']([A-Za-z0-9+\/_=-]{16,})["']\s*\)/gi;
    while ((m = re.exec(html)) != null && out.length < 12) {
        var ad = b64decode(m[1]);
        if (/^https?:\/\//i.test(ad)) add(ad);
    }
    /* Tambien aparecen como cadenas JS/JSON ya visibles, aunque no esten en iframe. */
    re = /(?:player_iframe|iframe|embed_url|stream_url|source|file|src)["']?\s*[:=]\s*["'](https?:\/\/[^"']+)["']/gi;
    while ((m = re.exec(html)) != null && out.length < 12) add(m[1]);
    re = /go_to_player\(['"]([^'"]+)/gi;
...
/* ---- Cuevana3e / proveedores modernos ----
 * Cuevana3e no entrega el HLS directamente: data-server apunta a un player.
 * Priorizamos endpoints que ya conocemos y dejamos el iframe como fallback.
 */
function exVidLink(url, label, ref) {
    var s = String(url || ""), h = hostOf(s), m, api, body, o, out = [], i, arr;
    if (h != "vidlink.pro" && h != "www.vidlink.pro") return [];
    m = /\/movie\/(\d+)/i.exec(s);
    if (m) api = "https://vidlink.pro/api/movie/" + m[1];
    else {
        m = /\/tv\/(\d+)\/(\d+)\/(\d+)/i.exec(s);
        if (m) api = "https://vidlink.pro/api/tv/" + m[1] + "/" + m[2] + "/" + m[3];
    }
    if (api) {
        body = httpGet(api, "https://vidlink.pro/");
        o = parseJson(body);
        if (o) {
            /* formatos vistos en distintas revisiones del API */
            if (o.stream_url) addSrc(out, mkSrc(o.stream_url, label + " HLS", "https://vidlink.pro/", /\.m3u8/i.test(o.stream_url) ? "hls" : "mp4"));
            if (o.url) addSrc(out, mkSrc(o.url, label, "https://vidlink.pro/", /\.m3u8/i.test(o.url) ? "hls" : (/\.mp4/i.test(o.url) ? "mp4" : "")));
            if (o.stream && typeof o.stream == "string") addSrc(out, mkSrc(o.stream, label, "https://vidlink.pro/", /\.m3u8/i.test(o.stream) ? "hls" : "mp4"));
            arr = o.sources || o.streams || o.data || [];
            if (typeof arr == "object") {
                if (!arr.length && arr.url) arr = [arr];
                for (i = 0; i < arr.length && i < 8; i++) if (arr[i]) {
                    var u = arr[i].url || arr[i].file || arr[i].src || arr[i].stream_url || "";
                    if (u) addSrc(out, mkSrc(u, label + (arr[i].quality ? " " + arr[i].quality : ""), "https://vidlink.pro/", /\.m3u8/i.test(u) ? "hls" : (/\.mp4/i.test(u) ? "mp4" : "")));
                }
            }
        }
        if (out.length) { log("    vidlink API -> " + out.length + " fuente(s)"); return out; }
    }
    /* Si cambia el API, el HTML sigue siendo un player resolvible por iframe/JS. */
    return exGeneric(url, label, ref, 1);
}

function exVsembed(url, label, ref, depth) {
    var h = httpGet(url, ref || "https://vsembed.ru/"), out = [], tags, i, a, links, j;
    if (!h) return [];
    out = scanMedia(h, label, url, url);
    if (out.length) return out;
    /* VidSrc suele envolver el reproductor real en un iframe; tambien puede esconderlo en comentarios. */
    tags = findTags(h, /^<iframe\\b/i);
    for (i = 0; i < tags.length && i < 6 && budgetLeft(); i++) {
        a = attrsOf(tags[i].tag);
        if (a["src"] || a["data-src"]) {
            var iu = absUrl(a["data-src"] || a["src"], url);
            var more = resolveEmbed(iu, label, url, (depth || 0) + 1);
            for (j = 0; j < more.length; j++) addSrc(out, more[j]);
            if (out.length) return out;
        }
    }
    /* patron data-hash + player_iframe usado por algunas versiones */
    var ph = /(?:player_iframe|iframe)["'\\s:=]+["']([^"']+)["']/i.exec(h);
    if (ph) {
        var pu = absUrl(ph[1], url), mm = resolveEmbed(pu, label, url, (depth || 0) + 1);
        for (j = 0; j < mm.length; j++) addSrc(out, mm[j]);
    }
    if (out.length) return out;
    links = discoverLinks(h, url);
    for (i = 0; i < links.length && i < 8 && budgetLeft(); i++) {
        var more2 = resolveEmbed(links[i], label, url, (depth || 0) + 1);
        for (j = 0; j < more2.length; j++) addSrc(out, more2[j]);
        if (out.length) break;
    }
    return out;
}

function exVidApi(url, label, ref, depth) {
    /* VidAPI cambia de dominio con frecuencia; el endpoint /embed es un player.
       No inventamos una URL de stream: seguimos iframe/JSON/media reales. */
    var h = httpGet(url, ref || (originOf(url) + "/")), out = [], links, i, j;
    if (!h) return [];
    out = scanMedia(h, label, url, url);
    if (out.length) return out;
    links = discoverLinks(h, url);
    for (i = 0; i < links.length && i < 8 && budgetLeft(); i++) {
        var more = resolveEmbed(links[i], label, url, (depth || 0) + 1);
        for (j = 0; j < more.length; j++) addSrc(out, more[j]);
        if (out.length) break;
    }
    return out;
...
function resolveEmbed(url, label, ref, depth) {
    ...
    if (h == "vidlink.pro" || h == "www.vidlink.pro") return exVidLink(url, label, ref);
    if (h == "vsembed.ru" || h == "www.vsembed.ru") return exVsembed(url, label, ref, depth);
    if (h == "vidapi.xyz" || h == "www.vidapi.xyz") return exVidApi(url, label, ref, depth);
...
function provJuanita(ctx) {
    var base = autoDomainEnsure("juanita", "https://pelisjuanita.com"), slugs = juanitaSlugCandidates(ctx), infoU = [], pageU = [], i, u, r, n = slugs.length, tried = {}, maybe = null, evals = [];
...
function parseCuevana(html, base) {
    ...
    for (i = 0; i < tags.length; i++) {
        ...
        if (!v) continue;
        if (!/^https?:|^\/|^player\.php/i.test(v)) {
            var d = b64decode(v.split("?v=")[1] || v);
            if (/^https?:\/\//i.test(d)) v = d; else continue;
        }
        var lang = "";
        for (j = 0; j < marks.length; j++) if (marks[j].at < tags[i].index && tags[i].index - marks[j].at < 2500) lang = marks[j].lang;
        v = absUrl(v, pbase);
        out.push(mkCand(v, lang, base + "/", "Cuevana3"));
    }
    /* Si el HTML cambia de atributo, no perder los players conocidos. */
    var urls = [], reu = /https?:\/\/[^\s"'<>]+/gi;
    while ((m = reu.exec(html || "")) != null && urls.length < 40) {
        var u = cleanUrl(m[0]);
        if (/(?:tungtungsahur\.cuevana3e\.pro|vsembed\.ru|vidlink\.pro|vidapi\.xyz|player\.videasy\.net)/i.test(u)) urls.push(u);
    }
    for (i = 0; i < urls.length; i++) {
        var exists = false;
        for (j = 0; j < out.length; j++) if (out[j].url == urls[i]) { exists = true; break; }
        if (!exists) out.push(mkCand(urls[i], "", base + "/", "Cuevana3"));
    }
    /* Orden deliberado: API/players que podemos resolver sin WebView primero;
       Videasy queda al final porque el sondeo detecta Cloudflare/captcha. */
    out.sort(function (a, b) {
        function rank(u) {
            if (/vidlink\.pro/i.test(u)) return 0;
            if (/vsembed\.ru/i.test(u)) return 1;
            if (/vidapi\.xyz/i.test(u)) return 2;
            if (/tungtungsahur\.cuevana3e\.pro/i.test(u)) return 3;
            if (/player\.videasy\.net/i.test(u)) return 9;
            return 5;
        }
...
function provCuevana(ctx) {
    ...
    for (fi = 0; fi < CUEVANA_FAMILIES.length && budgetLeft(); fi++) {
        ...
        for (bi = 0; bi < fam.bases.length && budgetLeft(); bi++) {
            var base = (fam.autoId ? autoDomainEnsure(fam.autoId, fam.bases[bi]) : fam.bases[bi]), baseSlugs = uniq([slugCuevana(ctx.titleEs), slugCuevana(ctx.titleEn)].concat(cuevanaAltSlugs(ctx))), slugs = [], reached = false;
...
function siteFind(site, ctx) {
    ...
    for (bi = 0; bi < site.bases.length && budgetLeft(); bi++) {
        var base = (site.id == "poseidonhd" ? autoDomainEnsure("poseidonhd", site.bases[bi]) : site.bases[bi]), reached = false;
...
function prefetchProviders(ctx) {
    ...
    if (slug1) {
        ...
        for (i = 0; i < s.length; i++) urls.push(ctx.kind == "movie" ? fam.movie(fam.autoId ? autoDomainEnsure(fam.autoId, fam.bases[0]) : fam.bases[0], s[i]) : fam.episode(fam.autoId ? autoDomainEnsure(fam.autoId, fam.bases[0]) : fam.bases[0], s[i], ctx.season, ctx.episode));
    }
    if (q0) {
        for (i = 0; i < SITES.length && n < 6; i++) {
            if (SITES[i].extra && !extraSites()) continue;
            urls.push((SITES[i].id == "poseidonhd" ? autoDomainEnsure("poseidonhd", SITES[i].bases[0]) : SITES[i].bases[0]) + SITES[i].search[0].replace("{q}", enc(q0).replace(/%20/g, "+")));
            n++;
        }
    }
    prefetchUrls(urls);
...
if (typeof source != "undefined") {
    source.enable = function (conf, settings, savedState) { _settings = settings || {}; _fail = {}; _okh = {}; _pre = {}; _tc = {}; _tcN = 0; _probeCache = {}; _srcDead = {}; _autoDomains = {}; _autoDomainAt = {}; autoDomainsInit(); };
