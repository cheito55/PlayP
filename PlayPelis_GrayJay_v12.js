...
var _probeCache = {};
var _srcDead = {};
var _force = {};        /* hosts que un proveedor pidio resolver aunque esten en UNSUPPORTED (TMDB-embeds) */

function log(s) { _debug += String(s) + "\n"; }
function resetDebug() { _debug = ""; }
function budgetLeft() { return Date.now() < _deadline; }
function startBudget() { _deadline = Date.now() + BUDGET_MS; _pre = {}; _force = {}; }
...
function idsInHtml(head, ctx) {
    ...
    /* ids dentro de URLs codificadas ?v=<base64>: se decodifican y se buscan tmdb=ID, /movie/ID, /tv/ID */
    var rv = /[?&]v=([A-Za-z0-9+\/=_\-]{20,})/g, nv = 0, dv, im;
    while ((m = rv.exec(head)) != null && nv++ < 12) {
        dv = b64decode(m[1]);
        if (!/^https?:/i.test(dv)) continue;
        im = /[?&]tmdb=(\d+)|\/(?:movie|tv)\/(\d+)/i.exec(dv);
        if (im) tm.push(im[1] || im[2]);
    }
    return { imdbN: ni, imdbHit: !!(ctx.imdbId && imdb[ctx.imdbId]), tmdbN: uniq(tm).length, tmdbHit: tm.indexOf(String(ctx.id)) >= 0 };
}
...
function batchRaw(urls, referer, extra) {
    var out = [], i, res, r, b, h, code, bt, nlog = 0;
    if (urls.length > 1 && typeof http.batch == "function" && budgetLeft()) {
        try {
            bt = http.batch();
            for (i = 0; i < urls.length; i++) bt = bt.GET(urls[i], hdr(referer || (originOf(urls[i]) + "/"), typeof extra == "function" ? extra(urls[i]) : extra), false);
            res = bt.execute();
            for (i = 0; i < urls.length; i++) {
                r = res[i]; h = hostOf(urls[i]); b = readBody(r); code = r && r.code ? r.code : 0;
                if (code == 404 || code == 410) { out.push(""); continue; }
                if ((code == 403 || code == 429) && (!b || b.length < 30000)) { if (nlog++ < 3) log("HTTP " + code + " " + urls[i].substring(0, 90) + " (bloqueo del sitio, largo " + (b ? b.length : 0) + ")"); out.push(""); markFail(h); continue; }
    ...
    for (i = 0; i < urls.length; i++) out.push(httpGet(urls[i], referer, typeof extra == "function" ? extra(urls[i]) : extra));
    return out;
}
...
function httpGet(url, referer, extra) {
    ...
    try {
        ...
        if (r && (r.code == 403 || r.code == 429) && (!b || b.length < 30000)) { log("HTTP " + r.code + " " + url.substring(0, 90) + " (bloqueo del sitio, largo " + (b ? b.length : 0) + ")"); markFail(h); return ""; }
...
function batchGet(urls, referer, extra) {
    ...
    if (todo.length) { got = batchRaw(todo, referer, extra); for (i = 0; i < todo.length; i++) out[idx[i]] = got[i]; }
    return out;
}
...
var UNSUPPORTED = ["waaw.", "netu.", "hqq.", "younetu.", "hqtv.", "biribup.", "cuevana3.download", "1fichier.", "mega.nz", "mediafire.", "drive.google.", "uptobox", "rapidgator", "nitroflare", "katfile", "turbobit", "ddownload", "gofile.io", "krakenfiles", "pixeldrain", "vidlink.pro", "videasy.net", "vsembed.", "vidapi.xyz"];   /* los ultimos 4 (por TMDB id): app JS / captcha / solo iframe, segun sondeo.json */
...
var WISH_FAMILY_RE = /streamwish|hlswish|wishembed|awish|dwish|embedwish|wishfast|sfastwish|flaswish|obeywish|cdnwish|strwish|hgcloud|hgplay|gradehg|vibuxer|swdyu|swhoi|hanerix|dhtpre|audinifer|jodwish|medixiru|playerwish|wishonly|hglink/i;
function normalizeEmbedUrl(u) {
    u = cleanUrl(u);
    var bm = /^(https?:\/\/[^\/?#]+)\/([a-zA-Z0-9]{10,14})(?:\.html)?\/?(?:[?#].*)?$/.exec(u);
    if (bm && WISH_FAMILY_RE.test(hostOf(u))) return bm[1] + "/e/" + bm[2];
...
function discoverLinks(html, pageUrl) {
    ...
    re = /atob\(\s*["']([A-Za-z0-9+\/=_\-]{20,})["']\s*\)/g;
    while ((m = re.exec(html)) != null && out.length < 12) { u = b64decode(m[1]); if (/^https?:\/\/[^\s"'<>]+$/i.test(u)) add(u); }
    re = /(?:player_iframe|embed_url|embedUrl|stream_url)["']?\s*[:=]\s*["'](https?:\/\/[^"']+)["']/gi;
    while ((m = re.exec(html)) != null && out.length < 12) add(m[1].replace(/\\\//g, "/"));
    re = /go_to_player\(['"]([^'"]+)/gi;
...
function exGeneric(url, label, ref, depth) {
    ...
    var fm = /^(https?:\/\/[^\/?#]+)\/(?:(?:f|d|v|file|download)\/)?([a-zA-Z0-9]{8,16})(?:\.html)?\/?(?:[?#].*)?$/.exec(url);
    if (fm && depth < 2) {
        log("    generico: sin video en la pagina de archivo, se prueba " + fm[1].replace(/^https?:\/\//, "") + "/e/" + fm[2]);
        var alt = resolveEmbed(fm[1] + "/e/" + fm[2], label, ref, depth + 2);
...
function resolveEmbed(url, label, ref, depth) {
    ...
    if (d) return [d];
    if (hostMatches(h, UNSUPPORTED) && !_force[h]) { log("  sin soporte: " + h); return []; }
...
function resolveCands(cands, out, prov) {
    ...
    for (i = 0; i < cands.length; i++) {
        ...
        if (c.url) {
            ...
            if (c.force) _force[hostOf(c.url)] = 1;
            if (!c.force && hostMatches(hostOf(c.url), UNSUPPORTED)) { log("  sin soporte: " + hostOf(c.url)); continue; }   /* no gastar uno de los MAX_CAND intentos */
        }
    ...
    list.sort(function (a, b) {
        ...
        if (!d) d = nextNameRank(a) - nextNameRank(b);
    ...
    for (i = 0; i < list.length && n < MAX_CAND && budgetLeft(); i++) {
        c = list[i];
        var label = (c.lang ? c.lang + " \u00b7 " : "") + (c.q ? c.q + " \u00b7 " : "") + prov + " \u00b7 " + (c.name || (c.url ? prettyHost(c.url) : "directo")), got = [], okList = [], badList = [];
        if (c.srcs) {
...
function juanitaFromSearch(sb, q, ctx, base, tried) {
    ...
    var rm2 = {};
    for (j = 0; j < fresh.length; j++) rm2[us[j]] = us[fresh.length + j];
    bb = batchGet(us, base + "/", juanitaXhr(rm2));
...
/* dominios de PelisJuanita en orden de preferencia. Al siguiente se pasa SOLO si el anterior nunca respondio (caido / cambio de dominio). */
var JUANITA_BASES = ["https://pelisjuanita.com"];
/* ---- dominios remotos (opcional) ----
   Si un sitio cambia de dominio se edita UN archivo JSON tuyo (p. ej. en GitHub raw) y el plugin lo toma solo (cache 6 h).
   Formato: {"juanita":["https://nuevo.com"], "poseidonhd":[...], "cuevana3e":[...], "cuevana3_eu":[...], "<id de SITES>":[...]}
   Los dominios del JSON van PRIMERO; los de este archivo quedan de respaldo. Solo se aceptan "https://host" (sin ruta).
   Si un dominio no responde, el plugin ya pasa solo al siguiente (marca el host como caido). No se prueban dominios inventados. */
var DOMAINS_URL = "";     /* p. ej. "https://raw.githubusercontent.com/<usuario>/<repo>/main/domains.json" (o el ajuste domainsUrl) */
var DOMAINS_TTL = 21600000, _domAt = 0;
function mergeBases(cur, extra) {
    var all = (extra || []).concat(cur || []), out = [], seen = {}, i, u;
    for (i = 0; i < all.length; i++) {
        u = String(all[i] || "").replace(/\/+$/, "");
        if (/^https:\/\/[a-z0-9][a-z0-9.\-]*(?::\d+)?$/i.test(u) && !seen[u.toLowerCase()]) { seen[u.toLowerCase()] = 1; out.push(u); }
    }
    return out;
}
function applyRemoteDomains() {
    var url = (_settings && _settings.domainsUrl) || DOMAINS_URL, now = Date.now(), body, d, i, id, n = 0;
    if (!url || (_domAt && now - _domAt < DOMAINS_TTL) || !budgetLeft()) return;
    _domAt = now;
    try { body = readBody(http.GET(url, { "User-Agent": UA, "Accept": "application/json" }, false)); d = body ? parseJson(body) : null; }
    catch (e) { log("dominios remotos -> " + e); return; }
    if (!d || typeof d != "object" || Array.isArray(d)) { log("dominios remotos: respuesta no valida"); return; }
    if (d.juanita) { JUANITA_BASES = mergeBases(JUANITA_BASES, d.juanita); n++; }
    for (i = 0; i < SITES.length; i++) if (d[SITES[i].id]) { SITES[i].bases = mergeBases(SITES[i].bases, d[SITES[i].id]); n++; }
    for (i = 0; i < CUEVANA_FAMILIES.length; i++) {
        id = CUEVANA_FAMILIES[i].bases.join(" ").indexOf("cuevana3e") >= 0 ? "cuevana3e" : "cuevana3_eu";
        if (d[id]) { CUEVANA_FAMILIES[i].bases = mergeBases(CUEVANA_FAMILIES[i].bases, d[id]); n++; }
    }
    log("dominios remotos aplicados (" + n + " sitios)");
}
function provJuanita(ctx) {
    var i, r, h;
    for (i = 0; i < JUANITA_BASES.length && budgetLeft(); i++) {
        h = hostOf(JUANITA_BASES[i]);
        if (hostDead(h)) { log("  " + h + " marcado como caido, se salta"); continue; }
        r = provJuanitaAt(JUANITA_BASES[i], ctx);
        if (r.length || _okh[h]) return r;
        log("  " + h + " no respondio" + (i + 1 < JUANITA_BASES.length ? ", se prueba el espejo" : ""));
    }
    return [];
}
function juanitaXhr(refMap) {
    return function (u) { return refMap[u] ? { "X-Requested-With": "XMLHttpRequest", "Accept": "*/*", "Referer": refMap[u] } : null; };
}
function provJuanitaAt(base, ctx) {
    var slugs = juanitaSlugCandidates(ctx), infoU = [], pageU = [], i, u, r, n = slugs.length, tried = {}, maybe = null, evals = [];
    ...
    var all = infoU.concat(pageU), sIdx = -1, refMap = {};
    for (i = 0; i < n; i++) refMap[infoU[i]] = pageU[i];
    if (queries.length) { sIdx = all.length; all.push(base + qEndpoint + enc(queries[0])); refMap[all[sIdx]] = base + (ctx.kind == "movie" ? "/movies/" : "/series/"); }
    /* slugs directos + buscador: UN solo viaje de red (antes el buscador era un plan B secuencial y tardio) */
    var bodies = batchGet(all, base + "/", juanitaXhr(refMap));
    for (i = 0; i < n; i++) {
        tried[slugs[i]] = 1;
...
var SITES = [
    /* PoseidonHD es Next.js y usa el TMDB id en la URL (sondeo): /pelicula/<id>/<slug>, /serie/<id>/<slug>/temporada/S/episodio/E.
       Su buscador (?s=) devuelve siempre la portada: NO se busca, se arma la URL con el id de TMDB (identidad exacta). */
    { id: "poseidonhd", name: "PoseidonHD", bases: ["https://www.poseidonhd2.co"], search: ["/?s={q}"], mode: "tmdbid",
      paths: { movie: "/pelicula/{id}/{slug}", ep: "/serie/{id}/{slug}/temporada/{s}/episodio/{e}" } },
...
var WP_REST_SEARCH = "/wp-json/wp/v2/search?search={q}&per_page=10";
function linksFromWpJson(body, base) {
    var arr = parseJson(body), out = [], i, x, u, path;
    if (!Array.isArray(arr)) return out;
    for (i = 0; i < arr.length && i < 20; i++) {
        x = arr[i];
        if (!x || !x.url) continue;
        u = absUrl(String(x.url), base); path = u.replace(/^https?:\/\/[^\/]+/, "");
        if (BAD_PATH.test(path)) continue;
        out.push({ url: u, titles: [clean(strip(String(x.title || ""))), slugWords(u)], type: RE_TV.test(path) ? "tv" : (RE_MOVIE.test(path) ? "movie" : "unknown"), year: "" });
    }
    return out;
}
/* ---- sitios Next.js con TMDB id en la URL (PoseidonHD y clones de cuevana.biz) ---- */
var NEXT_SKIP_KEYS = /^(?:episodes|seasons|related|recommended|similar|series|movies|blog|blogTutoriales|sliderItems|tabLastMovies|tabTopMovies|tabLastReleasedMovies|topMoviesDay|topMoviesWeek|cast|genres|images|downloads?)$/i;
var NEXT_URL_KEYS = ["result", "embed", "embed_url", "embedUrl", "player", "iframe", "link", "url", "src", "file"];
var NEXT_PREF = ["streamwish", "vidhide", "voesx", "voe", "streamtape", "filemoon", "mixdrop", "doodstream", "dood"];
function nextNameRank(c) {
    var n = String(c.name || "").toLowerCase(), i;
    for (i = 0; i < NEXT_PREF.length; i++) if (n.indexOf(NEXT_PREF[i]) >= 0) return i;
    return 20;
}
function fillPath(p, o) { return String(p).replace("{id}", o.id).replace("{slug}", o.slug).replace("{s}", o.s).replace("{e}", o.e); }
function tmdbIdUrls(site, base, ctx) {
    var slugs = uniq([trimDash(slugJuanita(ctx.titleEs)), trimDash(slugJuanita(ctx.titleEn)), trimDash(slugJuanita(ctx.titleOrig))]).slice(0, 3), out = [], i, p = ctx.kind == "movie" ? site.paths.movie : site.paths.ep;
    for (i = 0; i < slugs.length; i++) out.push(base + fillPath(p, { id: ctx.id, slug: slugs[i], s: ctx.season, e: ctx.episode }));
    out.push(base + fillPath(p, { id: ctx.id, slug: "", s: ctx.season, e: ctx.episode }).replace("//", "/").replace(/\/(temporada)/, "/$1").replace(/\/$/, ""));
    return uniq(out);
}
function isNextServerUrl(u, siteHost) {
    if (/\.(?:jpe?g|png|webp|gif|svg|ico|css|js)(?:[?#]|$)/i.test(u)) return false;
    if (/download\.php|themoviedb|image\.tmdb|facebook|twitter|telegram|whatsapp|instagram/i.test(u)) return false;
    if (hostOf(u) == siteHost && /^\/(?:pelicula|serie|peliculas|series|_next|genero|episodios?)(?:\/|$)/i.test(u.replace(/^https?:\/\/[^\/]+/, ""))) return false;
    return true;
}
/* recorre el JSON de __NEXT_DATA__ y junta los objetos que traen un enlace de reproductor (latino/castellano/subtitulado salen del nombre de la clave) */
function nextServers(node, lang, out, depth, siteHost) {
    var k, v, i, u, own;
    if (!node || depth > 9 || out.length > 40) return;
    if (Array.isArray(node)) { for (i = 0; i < node.length; i++) nextServers(node[i], lang, out, depth + 1, siteHost); return; }
    if (typeof node != "object") return;
    own = langOf(String(node.lang || node.language || node.idioma || "")) || lang;
    for (i = 0; i < NEXT_URL_KEYS.length; i++) {
        v = node[NEXT_URL_KEYS[i]];
        if (typeof v == "string" && /^(?:https?:)?\/\//i.test(v)) {
            u = v.indexOf("//") === 0 ? "https:" + v : v;
            if (isNextServerUrl(u, siteHost)) { out.push({ url: u, lang: own, q: String(node.quality || node.calidad || "").toUpperCase(), name: String(node.cyberlocker || node.server || node.servidor || node.name || "") }); break; }
        }
    }
    for (k in node) {
        if (!node.hasOwnProperty(k) || NEXT_SKIP_KEYS.test(k)) continue;
        v = node[k];
        if (v && typeof v == "object") nextServers(v, langOf(k) || own, out, depth + 1, siteHost);
    }
}
function nextCandidates(html, base, pageUrl, site) {
    var m = /<script id="__NEXT_DATA__"[^>]*>([\s\S]*?)<\/script>/i.exec(html || ""), data = m ? parseJson(m[1]) : null, found = [], out = [], seen = {}, i, e, c, host = hostOf(base);
    if (data && data.props && data.props.pageProps) nextServers(data.props.pageProps, "", found, 0, host);
    log("    Next.js: " + (data ? "__NEXT_DATA__ ok" : "sin __NEXT_DATA__") + ", " + found.length + " enlaces de reproductor");
    for (i = 0; i < found.length; i++) {
        e = found[i];
        if (seen[e.url]) continue;
        seen[e.url] = 1;
        if (e.name && hostMatches(e.name.toLowerCase() + ".", UNSUPPORTED)) continue;        /* netu, 1fichier... */
        c = mkCand(e.url, e.lang, pageUrl, site.name);
        c.q = e.q; c.name = e.name;
        out.push(c);
    }
    return out;
}
function provTmdbId(site, ctx) {
    var bi, base, urls, bodies, i, c;
    for (bi = 0; bi < site.bases.length && budgetLeft(); bi++) {
        base = site.bases[bi];
        if (hostDead(hostOf(base))) continue;
        urls = tmdbIdUrls(site, base, ctx);
        bodies = batchGet(urls, base + "/");
        for (i = 0; i < urls.length; i++) {
            if (!bodies[i]) continue;
            c = nextCandidates(bodies[i], base, urls[i], site);
            if (!c.length) c = pageCandidates(bodies[i], urls[i], base, { mode: "wp", name: site.name }, ctx);    /* respaldo: iframes / data-* de la propia pagina */
            log("  " + urls[i].substring(0, 100) + " -> " + c.length + " candidatos");
            if (c.length) return c;
        }
        if (_okh[hostOf(base)]) break;
    }
    return [];
}
function siteFind(site, ctx) {
    var qs = siteQueries(ctx), bi, qi, pi, j, pats = site.search.slice(0), tries = 0;
    if (site.mode == "wp" && pats.indexOf(WP_REST_SEARCH) < 0) pats.push(WP_REST_SEARCH);
    for (bi = 0; bi < site.bases.length && budgetLeft(); bi++) {
        var base = site.bases[bi], reached = false;
        for (qi = 0; qi < qs.length && budgetLeft(); qi++) {
            for (pi = 0; pi < pats.length && tries < 4; pi++) {
                var url = base + pats[pi].replace("{q}", enc(qs[qi]).replace(/%20/g, "+")), html = httpGet(url, base + "/");
                tries++;
                if (!html) continue;
                reached = true;
                var found = rankLinks(/^\s*[\[{]/.test(html) ? linksFromWpJson(html, base) : findLinks(html, base, ctx.kind), ctx, 3);
                if (found.length) { for (j = 0; j < found.length; j++) found[j].base = base; return found; }
                /* sin coincidencias en ESTE patron (puede ser la portada u otra pagina): se prueba el siguiente */
            }
        }
        if (reached) break;
    }
...
function provSite(site, ctx) {
    if (site.mode == "tmdbid") return provTmdbId(site, ctx);
...
function prefetchProviders(ctx) {
    ...
    if (q0) {
        for (i = 0; i < SITES.length && n < 6; i++) {
            if (SITES[i].mode == "tmdbid") continue;
...
/* Proveedores que aceptan el TMDB id directo (sondeo de Cuevana3e): no hay coincidencia de titulo posible. Se saben iframe/JS/captcha,
   por eso van SOLO como ultimo recurso, con tope de tiempo, y dejan la cadena de redirecciones en el log. */
function provTmdbEmbeds(ctx) {
    var id = enc(ctx.id), s = ctx.season, e = ctx.episode, i, c, out = [], list = ctx.kind == "tv"
        ? ["https://vidapi.xyz/embed/tv/" + id + "/" + s + "/" + e, "https://vsembed.ru/embed/tv?tmdb=" + id + "&season=" + s + "&episode=" + e + "&ds_lang=es", "https://vidlink.pro/tv/" + id + "/" + s + "/" + e]
        : ["https://vidapi.xyz/embed/movie/" + id, "https://vsembed.ru/embed/movie?tmdb=" + id + "&ds_lang=es", "https://vidlink.pro/movie/" + id];
    for (i = 0; i < list.length; i++) { c = mkCand(list[i], "", "", "TMDB-embed"); c.force = true; out.push(c); }
    return out;
}
function collectSources(ctx) {
    ...
    for (i = 0; i < SITES.length; i++) { if (SITES[i].mode == "tmdbid") plan.push(mk(SITES[i])); }
    plan.push({ n: "Cuevana3", f: provCuevana });
    for (i = 0; i < SITES.length; i++) { if (SITES[i].mode != "tmdbid" && (!SITES[i].extra || extraSites())) plan.push(mk(SITES[i])); }
    plan.push({ n: "TMDB-embeds", cap: 9000, f: function () {
        if (servers > 0) { log("  (se omite: ya hay fuentes)"); return []; }
        return provTmdbEmbeds(ctx);
    } });
    ...
    for (i = 0; i < plan.length; i++) {
        ...
        if (!staged && plan[i].n == "Cuevana3") { staged = true; try { prefetchProviders(ctx); } catch (e) { log("prefetchProviders -> " + e); } }
        log("> " + plan[i].n);
        var savedDl = _deadline;
        if (plan[i].cap) _deadline = Math.min(_deadline, Date.now() + plan[i].cap);
        try {
            var c = plan[i].f(ctx);
...
if (typeof source != "undefined") {
    source.enable = function (conf, settings, savedState) { _settings = settings || {}; _fail = {}; _okh = {}; _pre = {}; _tc = {}; _tcN = 0; _probeCache = {}; _srcDead = {}; _domAt = 0; };
