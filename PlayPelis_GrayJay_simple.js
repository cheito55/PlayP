...
function verdict(v, ctx) {
    ...
    return isAmbiguous(ctx) ? "no" : "maybe";
}
...
function resolveCands(cands, out, prov) {
    ...
    prefetchUrls(pre);
    var n = 0, good = 0, late = [], start = out.length;
...
function juanitaSlugCandidates(ctx) {
    ...
    return uniq(out).slice(0, 6);     /* 6 slugs x 2 paginas = 12 pedidos en una tanda */
}
...
function provJuanita(ctx) {
    var base = "https://pelisjuanita.com", slugs = juanitaSlugCandidates(ctx), infoU = [], pageU = [], i, u, r, n = slugs.length, tried = {}, maybe = null;
    for (i = 0; i < n; i++) { u = juanitaUrlsFor(base, slugs[i], ctx); infoU.push(u.info); pageU.push(u.page); }
    /* fragmento de servidores + pagina real de cada slug, todo en UNA tanda paralela */
    var bodies = batchGet(infoU.concat(pageU), base + "/");
    for (i = 0; i < n; i++) {
        tried[slugs[i]] = 1;
        r = juanitaEval(slugs[i], bodies[i], bodies[n + i], base, ctx);
        if (r.vd == "yes") { log("  slug OK (verificado por " + r.v.why + (r.v.year ? " " + r.v.year : "") + ": '" + r.v.pageTitle + "'): " + slugs[i]); logRows(r.items); return r.items; }
        if (r.vd == "maybe" && !maybe) maybe = r;
        if (r.vd == "no") log("  descartado slug=" + slugs[i] + " (" + (r.v.why || "homonimo sin evidencia de a\u00f1o/ID") + (r.v.pageTitle ? ") -> pagina real: '" + r.v.pageTitle + "'" : ")"));
    }
    if (maybe) { log("  slug aceptado SIN evidencia de a\u00f1o/ID (el titulo no tiene homonimos en TMDB): " + maybe.slug); logRows(maybe.items); return maybe.items; }
    log("  slugs probados: " + slugs.join(", "));
    /* plan B: buscador propio de PelisJuanita. Cada resultado se valida contra TMDB (año/ID) antes de usarlo:
       ANTES se tomaba el primer resultado sin verificar y por eso Pinocho 2019/2022 devolvian siempre la version de 1940 */
    var qEndpoint = ctx.kind == "movie" ? "/movies/search?s=" : "/series/search?s=", queries = uniq([ctx.titleEs, ctx.titleEn].concat(ctx.altTitles || [])).slice(0, 3), qi, seenSlug = {};
    for (qi = 0; qi < queries.length && budgetLeft(); qi++) {
        var sb = httpGet(base + qEndpoint + enc(queries[qi]), base + "/");
        if (!sb) continue;
        var cands = juanitaSearchCandidates(sb, ctx.kind, base);
        log("  buscador Juanita '" + queries[qi] + "' -> " + cands.length + " resultado(s) (" + (parseJson(sb) ? "JSON" : "HTML") + ")");
        var ranked = rankLinks(cands, ctx, 4), fresh = [], j;
        for (j = 0; j < ranked.length; j++) if (!tried[ranked[j].slug] && !seenSlug[ranked[j].slug]) { seenSlug[ranked[j].slug] = 1; fresh.push(ranked[j]); }
        ...
        var bb = batchGet(us, base + "/"), m2 = null;
        for (j = 0; j < fresh.length; j++) {
            r = juanitaEval(fresh[j].slug, bb[j], bb[fresh.length + j], base, ctx);
            log("  buscador Juanita: '" + (fresh[j].titles[0] || fresh[j].slug) + "' slug=" + fresh[j].slug + " -> " + r.items.length + " filas, " + r.vd + (r.v.why ? " (" + r.v.why + ")" : ""));
            if (r.vd == "yes") { logRows(r.items); return r.items; }
            if (r.vd == "maybe" && !m2) m2 = r;
        }
        if (m2) { logRows(m2.items); return m2.items; }
    }
    return [];
}
