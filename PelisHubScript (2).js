...
function exOkRu(url, label, ref) {
    var rawUrl = cleanUrl(url), id = (String(rawUrl).match(/(?:videoembed|video|live)\/(\d+)/) || String(rawUrl).match(/[?&](?:id|mid|videoId)=(\d+)/i) || [])[1];
    if (!id) { log("    ok.ru: sin id en " + rawUrl.substring(0, 80)); return []; }
    var R = "https://ok.ru/", out = [], h = "", tries = [
        "https://ok.ru/videoembed/" + id,
        "https://ok.ru/video/" + id,
        "https://m.ok.ru/video/" + id
    ], ti;
    for (ti = 0; ti < tries.length && !h && budgetLeft(); ti++) h = httpGet(tries[ti], ref || R);
    if (!h) return out;
    var m = /data-options=(?:"([^"]*)"|'([^']*)')/i.exec(h), meta = null, o, fv;
    if (m) {
        o = parseJson(htmlUnescape(m[1] != null ? m[1] : m[2]).replace(/\\u0026/g, "&").replace(/\\\//g, "/"));
        fv = o && o.flashvars;
        if (fv) {
            meta = fv.metadata;
            if (typeof meta == "string") meta = parseJson(htmlUnescape(meta).replace(/\\u0026/g, "&").replace(/\\\//g, "/"));
            if (!meta && fv.metadataUrl) {
                var mu = htmlUnescape(String(fv.metadataUrl)).replace(/\\u0026/g, "&").replace(/\\\//g, "/");
                mu = absUrl(mu, tries[ti - 1] || rawUrl);
                meta = parseJson(httpGet(mu, tries[ti - 1] || rawUrl)) || parseJson(httpPost(mu, "", tries[ti - 1] || rawUrl));
            }
        }
    }
    if (!meta) {
        /* plan B: metadata/manifesto directo en el HTML */
        var t = htmlUnescape(h).replace(/\\u0026/g, "&").replace(/\\\//g, "/").replace(/\\"/g, '"'), mm;
        mm = /"(?:hlsManifestUrl|hlsMasterPlaylistUrl|ondemandHls)"\s*:\s*"([^"]+)"/i.exec(t);
        if (mm) addSrc(out, mkSrc(mm[1], label + " HLS", tries[0], "hls"));
        if (!out.length) {
            var j = parseJson((/\{\s*"(?:videos|hlsManifestUrl)[\s\S]{20,}/i.exec(t) || [])[0]);
            if (j && j.videos) meta = j;
        }
        if (!meta && !out.length) log("    ok.ru: sin metadata (data-options " + (m ? "presente" : "ausente") + ") plan B -> 0");
    }
    if (!meta) return out;
    var hls = meta.hlsManifestUrl || meta.hlsMasterPlaylistUrl || meta.ondemandHls || "";
    if (hls) addSrc(out, mkSrc(hls, label + " HLS", tries[0], "hls"));
    var vids = (meta.videos || []).slice(0);
    vids.sort(function (a, b) { return (OK_RANK[b.name] || 0) - (OK_RANK[a.name] || 0); });
    var i;
    for (i = 0; i < vids.length; i++) if (vids[i].url) {
        var vu = htmlUnescape(String(vids[i].url)).replace(/\\u0026/g, "&").replace(/\\\//g, "/");
        addSrc(out, mkSrc(vu, label + " " + (OK_LABEL[vids[i].name] || vids[i].name || "MP4"), tries[0], "mp4"));
    }
    if (!out.length) {
        var direct = scanMedia(h, label, tries[0], tries[0]);
        for (i = 0; i < direct.length; i++) addSrc(out, direct[i]);
    }
...
function exVk(url, label, ref) {
    var raw = cleanUrl(url), m = /video(-?\d+)_(\d+)/i.exec(raw), oid = "", vid = "";
    if (m) { oid = m[1]; vid = m[2]; }
    else {
        oid = ((/[?&]oid=(-?\d+)/i.exec(raw) || [])[1] || "");
        vid = ((/[?&](?:id|video_id)=([0-9]+)/i.exec(raw) || [])[1] || "");
        if (!vid) {
            var mm = /(?:video|clip|embed)\/(?:-?\d+_)?(\d+)/i.exec(raw);
            if (mm) vid = mm[1];
        }
    }
    var u = raw;
    if (u.indexOf("video_ext.php") < 0 && vid) u = "https://vk.com/video_ext.php?oid=" + (oid || "0") + "&id=" + vid + ((/[?&]hash=([0-9a-f]+)/i.exec(raw) || [])[0] || "").replace(/^\?/, "&");
    var R = /vkvideo\.ru/i.test(hostOf(u)) ? "https://vkvideo.ru/" : "https://vk.com/";
    var h = httpGet(u, ref || R), out = [], list = [], re, k;
    if (!h && vid && u.indexOf("video_ext.php") >= 0) h = httpGet("https://vkvideo.ru/video_ext.php?oid=" + (oid || "0") + "&id=" + vid, ref || R);
    if (!h) return out;
    h = htmlUnescape(h).replace(/\\u0026/g, "&").replace(/\\\//g, "/");
    re = /["']url(\d{2,4})["']\s*:\s*["']([^"']+)["']/gi;
    while ((k = re.exec(h)) != null) list.push({ q: parseInt(k[1], 10), u: k[2] });
    /* Algunas versiones entregan url720/url1080 sin comillas alrededor de la clave. */
    re = /(?:^|[,{])\s*url(\d{2,4})\s*:\s*["']([^"']+)["']/gi;
    while ((k = re.exec(h)) != null) list.push({ q: parseInt(k[1], 10), u: k[2] });
    list.sort(function (a, b) { return b.q - a.q; });
    var hl = /["'](?:hls|hls_ondemand)["']\s*:\s*["']([^"']+)["']/i.exec(h) || /(?:^|[,{])\s*(?:hls|hls_ondemand)\s*:\s*["']([^"']+)["']/i.exec(h);
    if (hl) addSrc(out, mkSrc(hl[1], label + " HLS", R, "hls"));
    var i;
    for (i = 0; i < list.length; i++) addSrc(out, mkSrc(list[i].u, label + " " + list[i].q + "p", R, "mp4"));
    /* Fallback para VK/VK Video que ya no expone urlXXX en el HTML pero sí deja
       una URL multimedia en JSON/JS. No ejecutamos JS ni intentamos saltar controles. */
    if (!out.length) {
        var direct = scanMedia(h, label, u, u);
        for (i = 0; i < direct.length; i++) addSrc(out, direct[i]);
    }
    if (!out.length) {
        var err = /"(?:error|error_text|msg)"\s*:\s*"([^"]{3,160})"/i.exec(h);
...
function siteFind(site, ctx) {
    var qs = uniq([ctx.titleEs, ctx.titleEn, ctx.titleOrig]).slice(0, 3), bi, qi, pi;
...
function episodeLink(html, base, s, e) {
    ...
    var p2 = new RegExp("(?:temporada|season)-0*" + S + "[-/]+(?:episodio|episode|capitulo|cap)-0*" + E + "(?:[/?#]|$)", "i");
    var p3 = new RegExp("/season/0*" + S + "/episode/0*" + E + "(?:[/?#]|$)", "i");
    var p4 = new RegExp("(?:episode|episodio|capitulo|cap)[_-]?0*" + E + "(?:[/?#]|$)", "i");
    var ps = new RegExp("(?:season|temporada|s)[_-]?0*" + S + "(?:[/?#]|$)", "i");
    while ((m = re.exec(html)) != null) {
        if (p1.test(m[1]) || p2.test(m[1]) || p3.test(m[1]) || (p4.test(m[1]) && ps.test(m[1]))) return absUrl(m[1], base);
    }
    /* Algunas plantillas guardan el episodio en data-* o onclick, no en href. */
    var attrs = /<(?:a|div|li|button)[^>]*(?:data-(?:url|href|link|episode|player)|onclick)=["']([^"']+)["'][^>]*>/gi;
    while ((m = attrs.exec(html)) != null) {
        var v = htmlUnescape(m[1]).replace(/\\\//g, "/");
        if (p1.test(v) || p2.test(v) || p3.test(v) || (p4.test(v) && ps.test(v))) return absUrl(v, base);
    }
    return "";
}
