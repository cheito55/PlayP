...
function hardenSrc(c, s, reresolve) {
    ...
    if (first.ok) return s;
    log("    verificar: " + (first.code || "sin respuesta") + " en " + prettyHost(s.url) + ", reintentando");
    if (first.code == 403) {
        var okRef = findWorkingReferer(s.url, ref);
        if (okRef !== null) { s.requestModifier = reqMod(okRef || (originOf(s.url) + "/")); log("    verificar: referer alternativo funciono"); return s; }
    }
    markFail(hostOf(s.url));
    if (typeof reresolve == "function" && budgetLeft()) {
        ...
        if (fresh && fresh.length && !looksLikeErrorPage("")) {
            ...
            if (f2.ok) { log("    verificar: re-resolucion dio un nodo/token sano"); return fresh[0]; }
        }
    }
