...
function provJuanita(ctx) {
    ...
    if (ctx.kind == "tv" && slugs.length) {
        ...
        if (slugs.length > 1) urls.push(base + "/series/ver-serie/" + slugs[1] + "/" + (ctx.season < 10 ? "0" : "") + ctx.season + "x" + (ctx.episode < 10 ? "0" : "") + ctx.episode);
        urls.push(base + "/series/ver-serie/" + slugs[0]);
    }
    var bodies = batchGet(urls, base + "/");
    for (i = 0; i < bodies.length; i++) {
        if (i >= verSerieIdx && verSerieIdx >= 0) break;
        var items = parseJuanita(bodies[i], base);
        if (items.length) { log("  slug OK: " + urls[i]); return items; }
    }
    if (verSerieIdx >= 0) {
        var vi, vs, out, links, j, direct, k;
        for (vi = verSerieIdx; vi < bodies.length && vi <= verSerieIdx + 2; vi++) {
            if (!bodies[vi]) continue;
            vs = bodies[vi];
            out = [];
            links = discoverLinks(vs, urls[vi]);
            for (j = 0; j < links.length; j++) out.push(mkCand(links[j], "", base + "/", "Juanita"));
            direct = scanMedia(vs, "", base + "/", urls[vi]);
            for (k = 0; k < direct.length; k++) out.push({ url: "", lang: "", srcs: [direct[k]], prov: "Juanita" });
            if (out.length) { log("  ver-serie: " + urls[vi] + " -> " + out.length + " candidato(s)"); return out; }
        }
    }
...
function collectSources(ctx) {
    ...
    /* PRIORIDAD: PelisJuanita -> Cuevana3 -> PoseidonHD2. */
    ...
    try {
        var pose = null;
        for (i = 0; i < SITES.length; i++) if (SITES[i].id == "poseidonhd") { pose = SITES[i]; break; }
        if (pose) servers += resolveCands(provSite(pose, ctx), out, "PoseidonHD2");
    } catch (e3) { log("Poseidon ERROR " + e3); }

    /* Resto de catálogos: descubrimiento paralelo y sin corte por número de servidores. */
    for (i = 0; i < SITES.length; i++) if (SITES[i].id != "poseidonhd") sites.push(SITES[i]);
    ...
    finalOut.sort(function (a, b) {
        function rank(x) {
            ...
            if (/PoseidonHD/i.test(x)) return 2;
