...
function siteFind(site, ctx) {
    var qs = titleQueries(ctx), bi, qi, pi, best, all = [];
    for (bi = 0; bi < site.bases.length && budgetLeft(); bi++) {
        var base = site.bases[bi], urls = [], bodies, j;
        for (qi = 0; qi < qs.length && urls.length < 8; qi++) {
            for (pi = 0; pi < site.search.length && urls.length < 8; pi++)
                urls.push(base + site.search[pi].replace("{q}", enc(qs[qi]).replace(/%20/g, "+")));
        }
        bodies = batchGet(urls, base + "/");
        for (j = 0; j < bodies.length && budgetLeft(); j++) {
            if (!bodies[j]) continue;
            var links = findLinks(bodies[j], base, ctx.kind);
            best = pickBest(links, ctx);
            if (best) return { url: best.url, base: base };
            all = all.concat(links);
        }
        best = pickBest(all, ctx);
        if (best) return { url: best.url, base: base };
    }
    return null;
}
...
function collectSources(ctx) {
    ...
    /* Orden de descubrimiento: los sitios mas probables primero, pero PlPro SIEMPRE se consulta.
       Las variantes de cada sitio se lanzan mediante batchGet cuando es posible. */
    plan.push({ n: "PelisJuanita", f: provJuanita });
    plan.push({ n: "Cuevana3", f: provCuevana });
    for (i = 0; i < SITES.length; i++) { if (!SITES[i].extra || extraSites() || /^(poseidonhd|pelispedia|pelisplus|pelishouse|cinetux)$/i.test(SITES[i].id)) plan.push({ n: SITES[i].name, f: (function(site) { return function () { return provSite(site, ctx); }; })(SITES[i]) }); }
    /* PlPro no depende de que el scraping haya fallado: se busca siempre. */
    plan.push({ n: "PlPro", f: function () { return provPlPro(ctx); } });
    for (i = 0; i < plan.length; i++) {
        ...
        try {
            ...
            /* tras encontrar varias fuentes, seguimos solo con proveedores prioritarios/PlPro;
               evita convertir una pagina en una cadena de 30-40 requests. */
            if (servers >= WANT_SERVERS && i >= 5 && plan[i].n != "PlPro") break;
        } catch (e) { log("  ERROR " + e); }
    }
    var idx = [];
