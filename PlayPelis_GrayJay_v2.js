...
var MAX_CAND = 10;          /* embeds por proveedor; se cortan al hallar una fuente REAL */
...
function badEmbedUrl(u) {
    u = cleanUrl(u || '');
    if (!u) return true;
    if (/^(?:about:|javascript:|data:|blob:)/i.test(u)) return true;
    if (/^https?:\/\/about:blank/i.test(u)) return true;
    if (/^https?:\/\/[^\/]+\/about:blank(?:[?#]|$)/i.test(u)) return true;
    if (/^https?:\/\/[^\/]+\/(?:ads?|blank|empty)(?:[/?#]|$)/i.test(u)) return true;
    return false;
}
function discoverLinks(html, pageUrl) {
    ...
    function add(x) {
        ...
        if (!x) return;
        x = absUrl(x, pageUrl);
        if (!/^https?:\/\//i.test(x) || x == pageUrl || badEmbedUrl(x)) return;
...
function exVidLink(url, label, ref) {
    var s = String(url || ""), h = hostOf(s), m, api, body, o, out = [], i, arr, page, links, more, apis = [];
    if (h != "vidlink.pro" && h != "www.vidlink.pro") return [];
    m = /\/movie\/(\d+)/i.exec(s);
    if (m) apis.push("https://vidlink.pro/api/movie/" + m[1]);
    m = /\/tv\/(\d+)\/(\d+)\/(\d+)/i.exec(s);
    if (m) apis.push("https://vidlink.pro/api/tv/" + m[1] + "/" + m[2] + "/" + m[3]);
    for (i = 0; i < apis.length && budgetLeft(); i++) {
        api = apis[i];
        body = httpGet(api, "https://vidlink.pro/");
        if (!body) continue;
        out = scanMedia(body, label, "https://vidlink.pro/", api);
        o = parseJson(body);
        if (o) {
            if (o.stream_url) addSrc(out, mkSrc(o.stream_url, label + " HLS", "https://vidlink.pro/", /\.m3u8/i.test(o.stream_url) ? "hls" : (/\.mp4/i.test(o.stream_url) ? "mp4" : "")));
            if (o.url) addSrc(out, mkSrc(o.url, label, "https://vidlink.pro/", /\.m3u8/i.test(o.url) ? "hls" : (/\.mp4/i.test(o.url) ? "mp4" : "")));
            if (o.stream && typeof o.stream == "string") addSrc(out, mkSrc(o.stream, label, "https://vidlink.pro/", /\.m3u8/i.test(o.stream) ? "hls" : (/\.mp4/i.test(o.stream) ? "mp4" : "")));
            arr = o.sources || o.streams || (o.data && (o.data.sources || o.data.streams || o.data)) || [];
            if (typeof arr == "object") {
                if (!arr.length && arr.url) arr = [arr];
                for (var k = 0; k < arr.length && k < 10; k++) if (arr[k]) {
                    var u = arr[k].url || arr[k].file || arr[k].src || arr[k].stream_url || arr[k].playlist || "";
                    if (u) addSrc(out, mkSrc(u, label + (arr[k].quality ? " " + arr[k].quality : ""), "https://vidlink.pro/", /\.m3u8/i.test(u) ? "hls" : (/\.mp4/i.test(u) ? "mp4" : "")));
                }
            }
        }
        if (out.length) { log("    vidlink API -> " + out.length + " fuente(s)"); return out; }
    }
    /* El HTML puede contener directamente el HLS o un iframe del reproductor. */
    page = httpGet(s, ref || "https://vidlink.pro/");
    if (page) {
        out = scanMedia(page, label, s, s);
        if (out.length) return out;
        links = discoverLinks(page, s);
        for (i = 0; i < links.length && i < 6 && budgetLeft(); i++) {
            more = resolveEmbed(links[i], label, s, 1);
            for (var j = 0; j < more.length; j++) addSrc(out, more[j]);
            if (out.length) return out;
        }
    }
    return [];
...
function exVsembed(url, label, ref, depth) {
    ...
    if (ph) {
        var pu = absUrl(ph[1], url), mm = badEmbedUrl(absUrl(ph[1], url)) ? [] : resolveEmbed(pu, label, url, (depth || 0) + 1);
...
function exGeneric(url, label, ref, depth) {
    ...
    for (i = 0; i < links.length && budgetLeft() && out.length < 6; i++) {
        if (badEmbedUrl(links[i])) continue;
...
function resolveEmbed(url, label, ref, depth) {
    ...
    if (badEmbedUrl(url) || !/^https?:\/\//i.test(url) || depth > 3 || !budgetLeft()) return [];
    var h = hostOf(url), d;
