/*
 * PelisHub v5 - Juanita + deep embed scraping + active providers - GrayJay source (ES5)
 *
 * v5:
 *  - Mantiene todo el scraping/extractores de v4.
 *  - DooPlay AJAX + REST quedan dentro del flujo de resolución.
 *  - discoverLinks() recorre iframe/data-src/data-video/data-server/redirect/meta refresh.
 *  - deepUrls() inspecciona JSON/objetos anidados de player.
 *  - exGeneric() permite cadena de hasta 3 niveles antes de dar por perdida una fuente.
 *  - Flujo: pagina -> embed -> segundo embed/player -> HLS/MP4.
 *
...
function exGeneric(url, label, ref, depth) {
    var h = httpGet(url, ref || (originOf(url) + "/")), out, i, links, deep = [], j, more;
    if (!h) return [];

    /* 1) Primero buscamos HLS/MP4 directamente en HTML/JS/P.A.C.K.E.R. */
    out = scanMedia(h, label, url, url);
    if (out.length) return out;

    /* 2) Algunos embeds esconden la fuente en objetos/JSON anidados. */
    try {
        deepUrls(parseJson(h) || h, deep, 0);
    } catch (e0) {}

    for (i = 0; i < deep.length && budgetLeft() && out.length < 12; i++) {
        if (!deep[i] || deep[i] == url) continue;
        more = resolveEmbed(absUrl(deep[i], url), label, url, depth + 1);
        for (j = 0; j < more.length; j++) addSrc(out, more[j]);
    }
    if (out.length) return out;

    /* 3) VOE puede dejar el enlace dentro del HTML sin iframe visible. */
    out = voeFromHtml(h, url, label);
    if (out.length) return out;

    /*
     * 4) Cadena profunda:
     *    pagina -> iframe/data-src/redirect -> segundo embed -> player -> HLS/MP4.
     *    Se permite hasta profundidad 3.
     */
    if (depth >= 3) return [];
    links = discoverLinks(h, url);
    for (i = 0; i < links.length && budgetLeft() && out.length < 12; i++) {
        more = resolveEmbed(links[i], label, url, depth + 1);
        for (j = 0; j < more.length; j++) addSrc(out, more[j]);
    }
    return out;
}
...
function findLinks(html, base, kind) {
    ...
    while ((m = re.exec(html || "")) != null) {
        ...
        var around = html.substring(Math.max(0, m.index - 700), Math.min(html.length, m.index + m[0].length + 900));
        var yr = (/\b((?:19|20)\d\d)\b/.exec(around) || [])[1] || "";
        var c = byUrl[u];
...
function scoreLink(c, ctx) {
    ...
    if (c.year && ctx.year) {
        var cy = parseInt(c.year, 10), ty = parseInt(ctx.year, 10), d = Math.abs(cy - ty);
...
var CUEVANA_FAMILIES = [
    {
        /* Front-end actual: /serie/<slug>/temporada-S-episodio-E */
        bases: ["https://cuevanna.cc"],
        movie: function (base, slug) { return base + "/pelicula/" + slug; },
        episode: function (base, slug, s, e) { return base + "/serie/" + slug + "/temporada-" + s + "-episodio-" + e; },
        search: null
    },
    {
...
function titleVariants(ctx) {
    ...
    if (/fresh prince/i.test(en) || /pr[ií]ncipe del rap/i.test(base)) { add("The Fresh Prince of Bel-Air"); add("Fresh Prince of Bel Air"); add("El Principe del Rap en Bel Air"); }
...
function embedFromText(t) {
    ...
    m = /"(?:embed_url|embed|player_url|player|video_url|video|url|link|file|iframe|src|source|stream|location)"\s*:\s*"([^"]+)"/i.exec(t);
    if (m) return cleanUrl(m[1]);
...
function deepUrls(node, out, depth) {
    ...
    for (k in node) {
        ...
        /* Campos típicos de respuestas de players/APIs. */
        if (/^(?:embed_url|embed|player_url|player|video_url|video|url|link|file|iframe|src|source|stream|location)$/i.test(k)) {
            if (typeof node[k] == "string") {
                e = embedFromText(node[k]);
                if (e && out.indexOf(e) < 0) out.push(e);
            }
        }

        deepUrls(node[k], out, depth + 1);
...
function collectSources(ctx) {
    var out = [], i, sites = [], reqs = [], servers = 0;
    /* PRIORIDAD: PelisJuanita -> Cuevana3 -> proveedores secundarios activos -> PlPro. */
    ...
    /* Resto de catálogos: descubrimiento paralelo y sin corte por número de servidores. */
    for (i = 0; i < SITES.length; i++) sites.push(SITES[i]);
    for (i = 0; i < sites.length; i++) {
        var qs = titleVariants(ctx), si, qj, sk;
        for (si = 0; si < sites[i].bases.length; si++) for (qj = 0; qj < qs.length; qj++) for (sk = 0; sk < sites[i].search.length; sk++) {
            reqs.push({ site: sites[i], url: sites[i].bases[si] + sites[i].search[sk].replace("{q}", enc(qs[qj]).replace(/%20/g, "+")), referer: sites[i].bases[si] + "/" });
        }
    }
    reqs = reqs.slice(0, 140);
    var br = batchGetReqs(reqs), bestBySite = {}, bi;
    ...
    for (i = 0; i < sites.length && budgetLeft(); i++) {
        ...
        if (!f) continue;
        try { servers += resolveCands(provSite(sites[i], ctx, f), out, sites[i].name); } catch (e4) { log(sites[i].name + " ERROR " + e4); }
    }

    /* PlPro sigue siendo fallback paralelo, pero limitado para que no inunde Calidad. */
    try {
        var pc = provPlPro(ctx) || [], limited = [], pi;
        for (pi = 0; pi < pc.length && pi < MAX_PLPRO_CAND; pi++) limited.push(pc[pi]);
        servers += resolveCands(limited, out, "PlPro");
    } catch (e5) { log("PlPro ERROR " + e5); }

    var finalOut = [], seen = {}, i2, src;
    for (i2 = 0; i2 < out.length; i2++) {
        src = out[i2];
        if (!src || !src.url || seen[src.url]) continue;
        seen[src.url] = 1; finalOut.push(src);
    }
    finalOut.sort(function (a, b) {
        function rank(x) {
            x = String(x || "");
            if (/PelisJuanita/i.test(x)) return 0;
            if (/Cuevana3/i.test(x)) return 1;
            if (/PlPro/i.test(x)) return 9;
            return 3;
        }
